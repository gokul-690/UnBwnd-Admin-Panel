...
export default function ReviewModeration() {
  ...
  const fetchData = (tab = activeTab) => {
    setIsLoading(true);
    fetch(`https://unbwnd-backned1.onrender.com/api/admin/reviews?status=${tab}`, { cache: 'no-store' })
      .then(res => res.json())
      .then(data => { 
  ...
  const handleAction = async (id, action) => {
    ...
    try {
      const res = await fetch(`https://unbwnd-backned1.onrender.com/api/admin/reviews/${id}/${action}`, { method: 'PUT' });
      if (res.ok) {
